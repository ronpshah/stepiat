...
//Alauddeen Lababneh: changed triple click handler
...
window.CodeMirror = (function() {
 ...
 //***********************
                // Alauddeen Lababneh.  Triple click handler
                // Based on CodeFold by Daniel Glazman <daniel@glazman.org>     
        ...
        var  currentToken = editor.getTokenAt(cursorposition);
             ...
             if( currentToken.state.type == "openTag" ){
                    ...
                    var tag = null;
                    var pos = 0;
                    var endTagOnSameLine;
                 
                    var po = getPositionOfOpenTag(cursorposition) 
                  //  c.log(po)
                  
                    var st ={line :po.lineNumber,ch:po.start}
                    var end ={line :po.lineNumber,ch:po.start +1}
                  var lineLength = (cm.lineInfo( po.lineNumber).text).length
                   var dep = 0;
                   
                   for (i =po.start;i<=lineLength;i++)
                   {
                      var ch = editor.getRange(st , end )
                      if (ch == "<"){
                         var typ= editor.getTokenAt(end).state.type 
                         if(typ== "openTag")
                        { 
                            dep++}
                        else if(typ =="closeTag")
                        { dep-- }
                      }
                       if(dep==0 )
                           {
                           var tok = editor.getTokenAt(end)
                            endTagOnSameLine=true
                            objtripleRegion.st ={line :po.lineNumber,ch:po.start}
                             objtripleRegion.end ={line :po.lineNumber, ch:tok.start}
                            break;
                           }
                      st.ch= st.ch +1
                      end.ch= end.ch +1
                    ...
                    if (endTagOnSameLine)
                       ...
                       editor.setSelection( objtripleRegion.st ,objtripleRegion.end); 
                    ...
                    while (!found) {
                        ...
                        if (-1 == gtPos) { // end of start tag not in line
                            ...
                            // don't continue
                                ...
                                // check if really empty tag
                                 str = lineText.substr(slashPos, gtPos - slashPos + 1);
                        ...
                        if (found) {
                            ...
                            var openP = getOpen( currentToken, cursorposition.line)
                            ...
                            tag =openP.state.tagName
                    
                            if (tag) {
                             
                                objtripleRegion.st = {
                                    line :openP.lineNumber , 
                                    ch : openP.start
                                }
                    ...
                    if (found) {
                        ...
                        if(endTagOnSameLine){
                            depth = -1
                            var closePos =getClose(currentToken,cursorposition.line)
                           
                             depth = 0;
                            var match = lineText.match(endTag);
                        
                         
                   
                            var indexOfEndTag = lineText.search(endTag) ; // index of endTag
                            var endTaglength = endTag.length ; 
                            
                            
                            var endChOfendTag = indexOfEndTag + endTaglength
                            objtripleRegion.end = {
                                line :l -1, 
                                ch : endChOfendTag
                            } ;//Alan 
                   
                            editor.setSelection( objtripleRegion.st ,{ch:closePos.end +1 ,line:l-1}); 
                        }
                            ...
                            while (l < lastLine) {
                                lineText = cm.getLine(l);
                                 match = lineText.match(startTagRegExp);
                                ...
                                if (match) { 
                                    ...
                                    for (var i = 0; i < match.length; i++) {
                                         ...
                                         indexOfEndTag = lineText.search(endTag) ; // index of endTag
                                         endTaglength = endTag.length ; 
                                         endChOfendTag = indexOfEndTag + endTaglength
                                        objtripleRegion.end = {
                                            line :l, 
                                            ch : endChOfendTag
                                        } ; 
...
// 
 ...
 function getOpen(token ,lineNum) {
                    var prePosition ;
                    var type = token.state.type ;
                    // return null if this is a close tag // 
                    if (type == "closeTag" ){//|| token.string == ">"
                        ...
                        return  getOpen(editor.getTokenAt(prePosition) , lineNum);
                    // return null;  
                    }
    
                    if (type == "openTag"  &&  (/\S/.test(token.string))){
                        //found the open tag "<xxx "  and it is not an empty string. 
                        // check for empty string because CodeMirror treats a space as an openTag
                     
                        if(depth == 0)
                        {
                            token.lineNumber = lineNum ; // add lineNumber to the token object// 
                            return token;   
                        }
                        else
                        {
                    ...
                    if (token.start == 0 ){  
                        var lineLength = (editor.lineInfo(lineNum -1).text).length
                        prePosition = {
                            line : lineNum -1, 
                            ch : lineLength
                        };                  
                        return getOpen(editor.getTokenAt(prePosition) , lineNum -1);
                    }
                ...
                function getClose(token ,lineNum) {
                     var nextPosition ;
                    var type = token.state.type ;
                  
                    // return null if this is a close tag // 
                    if (type == "openTag"&&  (/\S/.test(token.string)) ){//|| token.string == ">"
                        ...
                        return  getClose(editor.getTokenAt(nextPosition) , lineNum);
                     }
                    ...
                    if (token.end == (editor.lineInfo(lineNum).text).length){  
                        ...
                        return getClose(editor.getTokenAt(nextPosition) , lineNum + 1);
                    }
