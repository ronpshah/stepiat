...
function fillAttTable( arrAllowedAttributes  ,elementX , insideOpen , isXMLValid ,cursorposition ,currentToken) {
    ...
    if(insideOpen && isXMLValid && currentToken.state.type != "closeTag")
    {
        ...
        // the first and last elements in arrOpenTagTokeons are : "<tagName " and ">" 
...
function createElementsTable(e, arrAllowedChildElements) {
	...
	$('#step-div-elem td').attr('title','Click-> Show documentation; &#013; Double click-> Add tag to tei editor;');
	$('#step-div-elem td').attr('title','Click-> Show documentation; \x0ADouble click-> Add tag to tei editor;');
...
//********************
		...
		if(freqElems[e.target.textContent] === undefined) 
			freqElems[e.target.textContent] = 1; 
		else freqElems[e.target.textContent] = freqElems[e.target.textContent] + 1;
		
			  
    });
	var validFreqElems = ValidFreqElements(freqElems, arrAllowedChildElements);	
		//console.log("Frequent Elems: " + validFreqElems)
		if(validFreqElems.length>0){
			createFreqElementsTable(e, validFreqElems);
		}
		//createFreqElementsTable(e, validFreqElems);
}

function createFreqElementsTable(e, validFreqElems) {
    // this removes the existing table and then create a new table.
    //$("#step-div-elem #frequently-used-elements #elementList").html("<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"display\" id=\"step-tbl-elem\"></table>");
   var $freqElementTable = $("#step-div-elem #elementList");
   
   $freqElementTable.off('dblclick').off('click');
   
   	/*if($freqElementTable.hasClass('dataTable')){
		$freqElementTable.fnDestroy();
	}*/
	var arrDataFreq = [];
	for(i=0;i<validFreqElems.length;i++){
		arrDataFreq.push([validFreqElems[i]]);
	}
	
    $freqElementTable.dataTable({
        "aaSorting": jsonConfig.tblElements.sort,
        "bFilter": false,
        "aaData": arrDataFreq,
		"bDestroy": true,
	...
	$('#step-div-elem #elementList td').attr('title','Click-> Show documentation; &#013; Double click-> Add tag to tei editor;');
	$('#step-div-elem #elementList td').attr('title','Click-> Show documentation; \x0ADouble click-> Add tag to tei editor;');
     
    // Click inside step-tbl-elem // Show documentation for the target element     
    $freqElementTable.click( function(e) {
        $(".item-documentation").html("&lt;<b> " + (e.target.textContent) +"</b>&gt;: " + getElemDocumentations(schemaX.HashNS["http://www.tei-c.org/ns/1.0"][e.target.textContent]));   
    }),
        
    $freqElementTable.dblclick( function(e) {
        // if cell is not empty
        if (e.target.textContent !=  jsonConfig.tblElements.oLanguage.sEmptyTable){
...
//********************
		...
		//console.log("selected freq elem: " + e.target.textContent);   
...
function createEntitiesTbl(createOptions){
	...
	if(!arrJson)
	{
       ...
       entityTable = $('#step-tbl-entites').dataTable( {
        ...
        $('#step-tbl-entites').on("dblclick", "td", function(e ) {
...
function createCommentsTbl(curMarker){
    ...
    { 
        ...
        var commentsTable = $('#step-tbl-comments').dataTable( {
            ...
            "bScrollInfinite": true ,
            "sScrollY": '315px',//jsonConfig.tblEntities.scrollheight,
            "bFilter": true ,
